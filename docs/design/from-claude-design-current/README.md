...
├── FULLBLEED_HANDOFF.md       ← v5 deck layout: full-bleed card (supersedes DESIGN §6 + MOTION §1.3)
...
└── mockups/
    ...
    └── betbook-full-bleed.html ← v5 full-bleed deck: sections, mid-swipe seam, nudge, reduced motion
```

## Order of work

1. Drop `flutter/` into `lib/theme/` and wire `AppTheme.light` / `AppTheme.dark` into `MaterialApp`.
2. Build the card deck (`FULLBLEED_HANDOFF.md` first, then `DESIGN_HANDOFF.md` §6 + `MOTION_HANDOFF.md` §1 for everything it does not override) — it is the navigation shell everything else lives in.
